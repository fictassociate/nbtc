                ...
                <div class="col-md-12">
                    <div class="portlet light bordered">
                        <div class="portlet-body form">
                            <!-- BEGIN FORM-->
                            <form action="{{ route('indicator.update', ['id'=>$data['indicator']->ID_MAST_INDICATOR]) }}" id="indicator-form" class="horizontal-form" method="POST">
                                @csrf
                                <div class="form-body">
                                    <div class="row">
                                        <div class="col-md-12">
                                            <div class="form-group">
                                                ...
                                                <input type="text" class="form-control" name="INDICATOR_NAME" value="{{ $data['indicator']->INDICATOR_NAME }}" placeholder="กรอกตัวชี้วัด" maxlength="255">
                                            </div>
                                        </div>
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                ...
                                                <select class=" select2 form-control" name="ID_MAST_SUBJECT" id="subject">
                                                    @foreach ($data['subject'] as $row)
                                                        <option value="{{ $row->ID_MAST_SUBJECT }}" {{ $data['subject_id']->ID_MAST_SUBJECT == $row->ID_MAST_SUBJECT ? 'selected' : '' }}>{{ $row->SUBJECT_NAME }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                                            ...
                                            <div class="form-group">
                                                ...
                                                <select class=" select2 form-control" name="ID_MAST_GOAL" id="goal">
                                                    @foreach ($data['goal'] as $row)
                                                        <option value="{{ $row->ID_MAST_GOAL }}" {{ $data['indicator']->ID_MAST_GOAL == $row->ID_MAST_GOAL ? 'selected' : '' }}>{{ $row->GOAL_NAME }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
